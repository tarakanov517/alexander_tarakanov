# Alexander Tarakanov

**Associate Professor, HSE University**  
**Research Scientist, AI VK**

---

## About Me

I am an Associate Professor at the Faculty of Computer Science, HSE University, and a Research Scientist at AI VK.

My research lies at the intersection of **Machine Learning**, **Scientific Computing**, and **Applied Mathematics**. I develop mathematically grounded machine learning methods for scientific applications and large-scale AI systems.

My current research interests include:

- Physics-Informed Machine Learning
- Scientific Machine Learning
- Representation Learning
- Spectral and Geometric Deep Learning
- Graph and Sheaf Neural Networks
- Recommender Systems

---

## Research Vision

My long-term goal is to develop machine learning methods that combine strong mathematical foundations with practical impact. I am particularly interested in incorporating physical laws, geometry, and optimization principles into modern AI systems.

---

## News

- **September 2026** — Presenting *Physics-Informed Neural Networks for Surrogate Modeling and Parameter-Space Generalization in Subsurface Flow* at the **European Conference on the Mathematics of Geological Reservoirs (ECMOR 2026)**, Porto, Portugal.

- **July 2026** — *PIEFS: Physics-Informed Eigenfunction Features with Learnable Scaling* accepted to the **AI4Physics Workshop @ ICML 2026**.

- **March 2026** — Launched a student research project on **sheaf-theoretic methods for Graph Neural Networks and Recommender Systems**, focusing on geometric message passing, learned sheaf structures, and representation learning for recommendation tasks.

---

## Selected Publications

A complete list of publications is available on my [Google Scholar profile](https://scholar.google.com/citations?user=ojshPuIAAAAJ&hl=ru).

- **PIEFS: Physics-Informed Eigenfunction Features with Learnable Scaling**  
  Varvara Nazarenkko, Timur Lidzhiev, Alexander Tarakanov.  
  *AI4Physics Workshop @ ICML 2026*, 2026.  
  [arXiv](https://arxiv.org/abs/2607.03692)

- **On Unbiased Estimation for Discretized Models**  
  Jeremy Heng, Ajay Jasra, Kody J. H. Law, Alexander Tarakanov.  
  *SIAM/ASA Journal on Uncertainty Quantification*, 11(2), 616–645, 2023.  
  [arXiv](https://arxiv.org/abs/2102.12230)

- **Regression-based Sparse Polynomial Chaos for Uncertainty Quantification of Subsurface Flow Models**  
  Alexander Tarakanov, Ahmed H. Elsheikh.  
  *Journal of Computational Physics*, 399, 108909, 2019.

---

## Current Projects

- Physics-Informed Machine Learning
- Scientific Machine Learning
- Geometric Representation Learning
- Sheaf Neural Networks
- AI for Physical Sciences

---

## Teaching

I teach courses in:

- Machine Learning
- Differential Equations
- Numerical Methods
- Machine Learning for Natural Sciences
- Recommender Systems

I supervise Master's students working in Machine Learning, Scientific AI, and Recommender Systems.

---

## Open to Collaboration

I am always interested in discussing research collaborations in:

- Scientific Machine Learning
- Physics-Informed AI
- Representation Learning
- Foundation Models for Science
- Geometric Deep Learning

Feel free to contact me if you are interested in collaborating.

---

## Links

- 📚 [Google Scholar](https://scholar.google.com/citations?user=ojshPuIAAAAJ&hl=ru)
- 📄 [arXiv](https://arxiv.org/search/advanced?advanced=&terms-0-operator=AND&terms-0-term=alexander+tarakanov&terms-0-field=all&classification-computer_science=y&classification-physics_archives=all&classification-include_cross_list=include&date-filter_by=all_dates&date-year=&date-from_date=&date-to_date=&date-date_type=submitted_date&abstracts=show&size=50&order=-announced_date_first)
- 💻 [GitHub](https://github.com/tarakanov517)
- 💼 [LinkedIn](https://www.linkedin.com/in/alexander-tarakanov-31145b48/)
- ✉ [Email](mailto:atarakanov@hse.ru)

---

## Contact

**Alexander Tarakanov**  
Associate Professor, HSE University  
Research Scientist, AI VK  

Email: [atarakanov@hse.ru](mailto:atarakanov@hse.ru)
